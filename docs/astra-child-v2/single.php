 ...
 * Página de post individual. Rank Math cuida do schema Article + breadcrumbs.
 */
...
if (have_posts()): the_post();
    $pid = get_the_ID();
    $cat = veb_get_cat($pid);
    $cat_slug = veb_get_cat_slug($pid);
    $cat_id = veb_get_cat_id($pid);
    $is_new = veb_is_new(get_the_date('Y-m-d H:i:s'));
    $trending_cats = veb_trending_cats();
    $is_hot = in_array($cat_id, $trending_cats, true);
    $reading = veb_reading_time($pid);
    ...
    /* Posts iniciais para "Continue Lendo" (8 mais recentes, exclui atual) */
    global $wpdb;
...
<div class="post-meta-row">
    <span class="cat-badge" style="background:<?php echo esc_attr(veb_cat_color($cat_slug)); ?>"><?php echo esc_html($cat); ?></span>
...
if (!empty($related_cats)):
    ...
    <section class="related-section">
        ...
        <div class="cards-grid">
            <?php foreach ($related as $rp) veb_render_card($rp); ?>
        </div>
    </section>
    <?php endif;
endif;
...
<section class="continue-section" aria-labelledby="continue-h">
    <div class="section-hdr"><h2 id="continue-h"><span aria-hidden="true">&#128214;</span> Continue Lendo</h2><span class="section-hdr-sub"><?php echo (int)$total_continue; ?> publicaç<?php echo $total_continue===1?'ão':'ões'; ?></span></div>
    <div class="cards-grid" id="allGrid" data-done="<?php echo $continue_has_more ? '0' : '1'; ?>">
        <?php foreach ($continue_posts as $cp) veb_render_card($cp); ?>
    </div>
