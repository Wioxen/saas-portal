<?php
/**
 * Astra Child — archive.php
 * Categorias, tags, autor, busca, datas. Grid + paginação numérica + infinite scroll opcional.
 */
...
/* Detectar tipo de archive e montar header */
$archive_title = '';
$archive_desc = '';
$archive_color = '#0d6844';
$archive_cat_id = 0;
$archive_search = '';
...
if (is_category()) {
    ...
    $archive_color = veb_cat_color($cat_obj->slug);
    ...
    $archive_search = get_search_query();
    $archive_desc = sprintf('%d resultado%s encontrado%s.', $total, $total !== 1 ? 's' : '', $total !== 1 ? 's' : '');
} elseif (is_author()) {
    $archive_title = 'Posts de ' . get_the_author();
} elseif (is_date()) {
    if (is_year()) {
        $archive_title = 'Arquivo: ' . get_the_date('Y');
    } elseif (is_month()) {
        $archive_title = 'Arquivo: ' . get_the_date('F \d\e Y');
    } elseif (is_day()) {
        $archive_title = 'Arquivo: ' . get_the_date('d \d\e F \d\e Y');
    } else {
...
<header class="archive-header" <?php if ($archive_color): ?>style="border-left-color:<?php echo esc_attr($archive_color); ?>"<?php endif; ?>>
...
<?php while (have_posts()): the_post(); $p = get_post(); veb_render_card($p); endwhile; ?>
</div>
...
<!-- Paginação numérica (fallback / SEO) -->
