<?php
/**
 * Astra Child Guia dos Cursos — single.php
...
if (have_posts()): the_post();
    $pid = get_the_ID();
    $cat = gdc_get_cat($pid);
    $cat_slug = gdc_get_cat_slug($pid);
    $cat_id = gdc_get_cat_id($pid);
    $is_new = gdc_is_new(get_the_date('Y-m-d H:i:s'));
    $trending_cats = gdc_trending_cats();
    $is_hot = in_array($cat_id, $trending_cats, true);
    $reading = gdc_reading_time($pid);
...
<div class="post-meta-row">
    <span class="cat-badge" style="background:<?php echo esc_attr(gdc_cat_color($cat_slug)); ?>"><?php echo esc_html($cat); ?></span>
...
if (!empty($related_cats)):
    ...
    <section class="related-section">
        ...
        <div class="cards-grid">
            <?php foreach ($related as $rp) gdc_render_card($rp); ?>
        </div>
    </section>
    <?php endif;
endif;
...
<section class="continue-section" aria-labelledby="continue-h">
    ...
    <div class="cards-grid" id="allGrid" data-done="<?php echo $continue_has_more ? '0' : '1'; ?>">
        <?php foreach ($continue_posts as $cp) gdc_render_card($cp); ?>
    </div>
...
<a href="<?php echo esc_url(get_permalink($next_post->ID)); ?>" class="next-post-slide" id="nextPostSlide" aria-label="Próximo post sugerido">
    <div class="next-post-slide-img"><img src="<?php echo esc_url($np_thumb ?: gdc_fallback_img()); ?>" alt="" loading="lazy" decoding="async"></div>
