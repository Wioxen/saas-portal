<?php
/**
 * Astra Child Guia dos Cursos — front-page.php
 */
...
$fcat = gdc_get_cat($featured_main->ID);
$fcslug = gdc_get_cat_slug($featured_main->ID);
$fcid = gdc_get_cat_id($featured_main->ID);
...
<a href="<?php echo esc_url(get_permalink($featured_main->ID)); ?>" class="hero-main" data-pid="<?php echo (int)$featured_main->ID; ?>" data-cat="<?php echo esc_attr($fcat); ?>" data-cat-slug="<?php echo esc_attr($fcslug); ?>" data-thumb="<?php echo esc_url($lcp_src ?: gdc_fallback_img()); ?>" data-title="<?php echo esc_attr($featured_main->post_title); ?>">
    <div class="hero-main-body">
        <div class="badges">
            <span class="cat-badge" style="background:<?php echo esc_attr(gdc_cat_color($fcslug)); ?>"><?php echo esc_html($fcat); ?></span>
            <?php if (gdc_is_new($featured_main->post_date)): ?><span class="new-badge">NOVO</span><?php endif; ?>
    ...
    <div class="hero-main-img"><img src="<?php echo esc_url($lcp_src ?: gdc_fallback_img()); ?>" <?php if($lcp_srcset):?>srcset="<?php echo esc_attr($lcp_srcset); ?>" sizes="(max-width:768px) 100vw, 62vw"<?php endif;?> alt="<?php echo esc_attr($featured_main->post_title); ?>" width="800" height="450" fetchpriority="high" decoding="sync"></div>
    <div class="hero-main-footer">
        <time datetime="<?php echo esc_attr(date('c', strtotime($featured_main->post_date))); ?>"><?php echo esc_html(gdc_time_ago($featured_main->post_date)); ?></time>
        <span class="dot">·</span>
        <span class="reading-time"><?php echo (int)gdc_reading_time($featured_main->ID); ?> min de leitura</span>
    </div>
</a>
...
<?php foreach ($featured_secondary as $i => $p):
    ...
    $pc = gdc_get_cat($p->ID);
    $pcsl = gdc_get_cat_slug($p->ID);
    $pcid = gdc_get_cat_id($p->ID);
    $sexc = gdc_get_excerpt($p);
    $isn = gdc_is_new($p->post_date);
    $ish = in_array($pcid, $trending_cats, true);
?>
<a href="<?php echo esc_url($pl); ?>" class="hero-card" data-pid="<?php echo (int)$p->ID; ?>" data-cat="<?php echo esc_attr($pc); ?>" data-cat-slug="<?php echo esc_attr($pcsl); ?>" data-thumb="<?php echo esc_url($pt ?: gdc_fallback_img()); ?>" data-title="<?php echo esc_attr($p->post_title); ?>">
    <div class="hero-card-body">
        <div class="badges">
            <span class="cat-badge" style="background:<?php echo esc_attr(gdc_cat_color($pcsl)); ?>"><?php echo esc_html($pc); ?></span>
    ...
    <div class="hero-card-img"><img src="<?php echo esc_url($pt ?: gdc_fallback_img()); ?>" alt="<?php echo esc_attr(wp_trim_words($p->post_title, 6)); ?>" width="400" height="225" loading="<?php echo $i<2?'eager':'lazy'; ?>" decoding="async"></div>
    <div class="hero-card-footer"><time datetime="<?php echo esc_attr(date('c', strtotime($p->post_date))); ?>"><?php echo esc_html(gdc_time_ago($p->post_date)); ?></time></div>
</a>
<?php endforeach; ?>
</div>
</section>
...
<section class="smart-section" aria-labelledby="popular-h">
    ...
    <div class="smart-grid">
        <?php foreach ($most_read as $idx => $mp):
            ...
            $mc = gdc_get_cat($mp->ID);
            $mcs = gdc_get_cat_slug($mp->ID);
        ?>
        <a class="smart-card" href="<?php echo esc_url($ml); ?>" data-pid="<?php echo (int)$mp->ID; ?>" data-cat="<?php echo esc_attr($mc); ?>" data-cat-slug="<?php echo esc_attr($mcs); ?>" data-thumb="<?php echo esc_url($mt ?: gdc_fallback_img()); ?>" data-title="<?php echo esc_attr($mp->post_title); ?>">
            <span class="smart-card-rank"><?php echo (int)($idx+1); ?></span>
            <div class="smart-card-img"><img src="<?php echo esc_url($mt ?: gdc_fallback_img()); ?>" alt="" loading="lazy" decoding="async" width="400" height="225"></div>
            <div class="smart-card-body"><span class="smart-card-cat" style="background:<?php echo esc_attr(gdc_cat_color($mcs)); ?>"><?php echo esc_html($mc); ?></span><h4><?php echo esc_html(wp_trim_words($mp->post_title, 12)); ?></h4></div>
        </a>
        <?php endforeach; ?>
    </div>
</section>
...
<div class="quick-grid-wrap">
    ...
    <div class="quick-grid">
        ...
        <a href="<?php echo esc_url(get_category_link($cat->term_id)); ?>" class="quick-tag" data-cat-slug="<?php echo esc_attr($cat->slug); ?>" style="background:<?php echo esc_attr(gdc_cat_color($cat->slug)); ?>">
...
<div class="cards-grid" id="allGrid" data-done="<?php echo $has_more ? '0' : '1'; ?>"><?php foreach ($all_posts_page1 as $p) gdc_render_card($p); ?></div>
<?php if ($has_more): ?>
