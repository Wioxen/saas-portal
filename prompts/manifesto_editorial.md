...
> **REGRAS DE FORMATAÇÃO MOBILE-FIRST (não negociáveis):**
> - **Travessões longos `—` e `–` PROIBIDOS no corpo.** Use `:` (label/valor em listas), `,` (aposto curto), `(...)` (parêntese), ou ponto + frase nova (não no mesmo parágrafo).
> - **Parágrafos com no máximo 2 sentenças**, idealmente 120-180 caracteres cada. Bloco grande no mobile mata leitura.
> - **Mudou de ideia/foco → abre `<p>` novo.** Cada parágrafo é uma unidade visual no mobile.
> - **Quando ensina passo-a-passo de inscrição/candidatura**, o item 1 da lista ordenada DEVE conter URL clicável da fonte oficial entre parênteses após o nome da instituição. Ex: `<li>Acessar o site oficial do IFSP (<a href="https://www.ifsp.edu.br/ingresso">ifsp.edu.br/ingresso</a>) entre...`
...
## ATRIBUIÇÃO DE FONTE — APENAS NO RODAPÉ (regra anti-citação)

A informação no CORPO do artigo deve **parecer consolidada** — padrão jornalístico de autoridade. **Atribuição de fonte (portal/site/jornal/blog/veículo) vai SOMENTE no rodapé "Fonte: ..." (1 vez, no fim).**

### ❌ PROIBIDO no corpo do artigo

- "Segundo o portal X..."
- "De acordo com o site Y..."
- "Conforme noticiado pelo blog Z..."
- "O portal A informou que..."
- "Segundo a publicação..."
- "Segundo a reportagem..."
- "Segundo a divulgação oficial do [veículo]..."
- "Conforme o jornal..."
- "Publicado pelo portal..."
- "Divulgado pelo veículo..."
- Citação direta de nome de veículo: "Segundo o **G1 Bahia**", "Conforme a **Folha**", "Segundo o **Estadão**"

### ✅ PERMITIDO no corpo (fonte oficial/instituição)

- "Segundo o **INEP**" / "Segundo o **MEC**" / "Segundo a **Caixa**" / "Segundo o **Senai**"
- "De acordo com o **edital**" / "Conforme a **Portaria nº X**"
- "O **Ministério da Educação** informou..."
- "A **Fundação José Silveira** abriu..."

**Regra prática:** atribuição a ÓRGÃO PÚBLICO / INSTITUIÇÃO / EMPRESA é OK (são FONTES PRIMÁRIAS). Atribuição a VEÍCULO DE IMPRENSA é PROIBIDA no corpo (são FONTES SECUNDÁRIAS — ficam no rodapé).

### ⚠️ REGRA AMPLIADA — qualquer "O portal X / O site X / A redação X / O blog X" + verbo é PROIBIDO no corpo

Mesmo se o "X" não for veículo conhecido (ex: "EstágioTrainee.com", "BlogConcursoX"), a construção é proibida:

❌ **PROIBIDO:**
- "O portal EstágioTrainee.com analisou 4 opções"
- "A redação BlogVagasX informou que..."
- "O site CursosOnline destacou..."
- "O blog AprendaIA cita..."

✅ **CERTO — escreva COMO SE FOSSE A REDAÇÃO DO SITE EM USO** (guiadoscursos, vagasebeneficios, cursosenac, etc). A fonte scrapeada serve só como **BASE de dados** pra alimentar nossa análise. Nunca cite o veículo terceiro:

**Estilo PREFERIDO — primeira pessoa do plural (redação do site):**
- "**Reunimos** as 4 melhores opções de cursos de IA disponíveis em 2026."
- "**Selecionamos** 4 plataformas com aulas em português e certificado."
- "**Comparamos** carga horária, preço e certificado dos 4 principais cursos."
- "**Identificamos** 480 vagas distribuídas em 22 cursos da Fundação Bradesco em Salvador."

**Estilo aceitável — sujeito ativo do FATO (sem mencionar redação):**
- "Os 4 cursos comparados oferecem certificado..."
- "As 480 vagas estão distribuídas em 22 cursos de curta duração..."
- "A Fundação Bradesco abriu inscrições para 480 vagas em Salvador..."

**Estilo aceitável — voz passiva impessoal (último recurso):**
- "Foram analisadas 4 opções de cursos..."
- "Foram identificadas 480 vagas..."

**Por quê:** o artigo é uma **PUBLICAÇÃO da redação do site em uso** (com persona definida). A fonte scrapeada é INSUMO de dados, não COAUTORA. Mencionar veículo terceiro **transfere autoridade pra ele** e diz ao Google "esse aqui é só agregador". O leitor deve sentir que está LENDO a análise do nosso site — não um link pra outro lugar.

**Hierarquia de preferência:**
1. **Primeira pessoa do plural** ("Reunimos", "Selecionamos", "Comparamos", "Identificamos", "Analisamos")
2. **Sujeito ativo do FATO** (a Fundação X abriu, o Senai oferece, o MEC publicou)
3. **Voz passiva impessoal** ("Foram analisadas", "Foram abertas") — só quando 1 e 2 não cabem

Exceções: ÓRGÃOS oficiais (Senai, Inep, MEC), EMPRESAS sujeito do fato (Petrobras, Vale, Fundação Bradesco) — esses SÃO o fato, não fontes secundárias.

### Por que

- Menos atribuição visível = mais **autoridade percebida** (parece informação consolidada)
- Melhor retenção do leitor (não distrai pra "buscar a fonte")
- Padrão jornalístico de qualidade — fonte vai no rodapé, não no meio do texto
- Google premia conteúdo que parece **referência primária**, não que parece "agregador"
...
- **`<strong>` (NEGRITO ESTRATÉGICO — alta densidade obrigatória):** marcar TODOS os termos abaixo. Meta: 1+ `<strong>` por parágrafo + 2-4 por tabela/snippet/RD/FAQ.
  - **Números, valores, percentuais:** `R$ 1.518`, `R$ 8.105`, `1.000 vagas`, `90%`, `48 meses`, `1,99% ao mês`
  - **Datas e prazos:** `31 de janeiro`, `20 de dezembro de 2026`, `nesta sexta (8)`
  - **Nomes de programas/órgãos/entidades:** `Sisu`, `Prouni`, `Fies`, `Inep`, `MEC`, `Caixa Tem`, `CadÚnico`, `Desenrola 2.0`, `Pronampe`, `ProCred`, `MRS Logística`, `SENAI`, `Petrobras`, `Vale`
  - **Localidades-chave:** `Belo Horizonte`, `Manaus`, `Espírito Santo`, `Grande Vitória`
  - **Termos de keyword/intenção** (variações da palavra-chave principal nos primeiros parágrafos): se keyword é "Desenrola 2.0", primeira ocorrência em P1 vai em `<strong>`
  - **Cargos/produtos/categorias específicas:** `Aprendiz`, `Analista de Trânsito`, `Eletromecânica`, `Auxílio mensal`
  - Onde aplicar com PRIORIDADE: snippet, RD, FAQ items (resposta), tabelas, primeiro parágrafo após cada H2
  - **NÃO marcar:** verbos genéricos, conectores, preposições, frases inteiras (negrito de >5 palavras seguidas é spam visual)

### REGRA DE OURO: QUALIDADE DOS FATOS > QUANTIDADE DE PALAVRAS (sem limite)

**NÃO HÁ LIMITE de palavras por frase ou por parágrafo.** A obrigação é entregar **conteúdo de autoridade** com **densidade máxima de fatos, palavras-chave e dados verificáveis** — quanto mais informação relevante, melhor pra **autoridade SEO** e ranqueamento Google.

**Princípios:**
- Cada frase deve carregar 1+ FATO concreto (número, data, valor, entidade, regra, mecanismo)
- Cada parágrafo deve cobrir 1 ideia substantiva sem cortes artificiais
- Palavras-chave do tema aparecem com naturalidade (densidade ideal 1-2.5%)
- NUNCA truncar substância pra atender qualquer métrica de tamanho

**Referência ÚNICA de leitura mobile (não é limite):**
- Idealmente cada parágrafo cabe em 4-5 linhas mobile (~40-60 palavras é uma faixa confortável)
- Se a substância exige 80-100+ palavras, mantenha — qualidade prevalece
- Use `<strong>` em dados críticos pra criar pontos de fixação visual no parágrafo longo
- Use `:` ou `;` pra concatenar ideias relacionadas (não obrigatório, mas natural)

**Exemplos de parágrafos de autoridade (sem limite — qualidade prevalece):**

✅ **DENSO E SUBSTANTIVO (60+ palavras, todos fatos):**
> "O grande volume de **8.009 vagas** abertas pelo **Cetam Manaus** nesta terça pressiona a escolha do curso desde as **7h até as 23h59 de quarta (6)**, com seleção por **ordem de inscrição** nas **UEPT da capital amazonense** e exigências específicas de **CPF e validação no Portal do Candidato** — o que historicamente trava parte dos cadastros antes mesmo da confirmação."

✅ **CAUSA + DETALHE COMPLETO (80+ palavras):**
> "A nova **Renegociação Desenrola 2.0** anunciada pelo **governo federal** estabelece teto de **R$ 15 mil**, juros de **1,99% ao mês** e prazo de **48 meses** para dívidas vencidas há 90 dias até **31 de janeiro de 2026**, com descontos que variam de **30%-40% para 90-120 dias** de atraso até **80%-90% para débitos de 1-2 anos**: estrutura que beneficia particularmente quem tem renda comprovada de até **R$ 8.105** e dívidas mais antigas no cadastro do **Serasa** ou **SPC**."

**O que importa:**
1. ✅ Fatos concretos amarrados em frases substantivas
2. ✅ Palavras-chave naturalmente presentes (densidade SEO)
3. ✅ `<strong>` em dados críticos pra escaneabilidade
4. ✅ Parágrafo entrega valor completo sem cortes
5. ❌ NUNCA cortar substância pra "ficar curto"

**Métrica abandonada:** "frases ≤20 palavras", "parágrafo ≤40 palavras". Sem hard limit.
...
### H2 — REGRA DE AUTORIDADE (≠ regra do título principal)

**Distinção crítica:** título principal (`<h1>` / `title`) PODE usar verbos coloquiais alto-CTR pra Discover ("barra", "elimina", "derruba"). H2s do CORPO **NÃO PODEM** — Google lê H2s como sinal de **topical authority** e penaliza estrutura clickbait/sensacional no SERP.

**Verbos PROIBIDOS em H2** (são CTR de manchete, não autoridade de seção):
- ❌ derruba, barra, elimina, trava, pega, mata, queima, despenca, estoura, trava
- ❌ "X que mata", "Y que destrói", "Z que arrasa"
- ❌ adjetivos sensacionais: "absurdo", "chocante", "inacreditável", "incrível"

**Verbos PREFERIDOS em H2** (descritivos, ancorados em fato, com keyword longtail):
- ✅ "como funciona", "o que muda", "quando começa", "quem tem direito a", "critérios para"
- ✅ "calendário de", "valores de", "prazos do", "regras do", "etapas do"
- ✅ "afeta", "determina", "exige", "estabelece", "regula", "define", "concede"

**Comparativo (mesmo conteúdo, registros diferentes):**

| ❌ H2 sensacional (não ranqueia SERP) | ✅ H2 autoritativo (ranqueia SERP) |
|---|---|
| "Erro no CadÚnico **derruba** pedidos do Fies e bolsas do Prouni" | "**CadÚnico desatualizado é a principal causa de indeferimento** no Fies e Prouni" |
| "3 critérios que **barram** candidatos no Cetam" | "**Critérios de elegibilidade do Cetam Manaus**: o que verificar antes da inscrição" |
| "**Erro** que **elimina** quem se inscreve no ENEM" | "**Como o cadastro Inep determina** a validação da inscrição no ENEM" |
| "Salário **despenca** quem não fez a prova" | "**Faixa salarial e impacto** da formação técnica no Programa Vale" |

**Estrutura ideal de H2 (autoridade SERP):**
- 6-12 palavras (ideal pra featured snippet)
- Contém keyword longtail da intenção de busca
- Verbo descritivo (não sensacional)
- Específica entidade/órgão/programa quando aplicável (Cetam, MEC, Inep, Vale)
- Funciona como pergunta implícita do usuário (intent-match)

**Teste do H2:**
1. Funciona como featured snippet do Google? → SIM = autoridade
2. Soa como manchete de tabloide? → NÃO = autoridade
3. Tem keyword + entidade + ângulo descritivo? → SIM = autoridade
...
- **Veículo de imprensa: ZERO menção** (atualizado 2026-05-14). Nem no corpo, nem no rodapé, nem em schema `citation`. Toda e qualquer publicação é **de autoria e auditoria da redação do site**. Scrape de g1/A Tarde/Metrópoles/BNews/Hora Brasil/CNN/etc alimenta o brain mas NÃO vira atribuição derivada. Fonte primária INSTITUCIONAL (gov.br, Inep, MEC, Alesp, Semob, CECIERJ, IFs) PODE ser citada — é a entidade COBERTA, não fonte derivativa.
...
### BACKLINKS INTERNOS (WORDPRESS) — REGRA DE OURO ATUALIZADA 2026-05-05

**Prioridade absoluta: AUTORIDADE do artigo > backlinks internos.** Backlink forçado em contexto irrelevante = penalidade SEO Google. **Melhor 0 backlinks do que 3 forçados.**

#### NUNCA forçar
- ❌ NÃO inventar/adicionar dados pra justificar backlink
- ❌ NÃO criar contexto artificial pra inserir link ("Em linha com [tema-irrelevante]...")
- ❌ NÃO existe posição obrigatória (final P1, meio H2, etc) — abolido
- ❌ NÃO existe quantidade mínima — pode ser 0, 1 ou 2

#### SÓ inserir backlink interno SE:
1. ✅ O artigo já menciona NATURALMENTE o tema/entidade do backlink
2. ✅ A inserção surge orgânica na frase, sem reescrever pra justificar
3. ✅ Anchor text = título do post (ou núcleo informativo) — nunca "clique aqui"
4. ✅ Backlink é PARTE de uma frase editorial completa

#### Exemplos

❌ **FORÇADO** (artigo sobre auxílio-alimentação inserindo backlink sobre Taxa das Blusinhas):
> "O salto do auxílio-alimentação para R$ 1.192 soma-se a medidas de cuidado vistas em **Taxa das blusinhas: governo diz que medida protege milhões de empregos**."

✅ **CONTEXTUAL** (artigo sobre auxílio-alimentação inserindo backlink sobre Licença-paternidade — tema afim de servidores):
> "O salto do auxílio-alimentação para R$ 1.192 soma-se a medidas de cuidado vistas em **Licença-paternidade de 20 dias para servidores federais**."

✅ **CONTEXTUAL** (artigo sobre Bolsa Família inserindo backlink sobre Vale-Gás — programa social complementar):
> "Famílias que recebem Bolsa Família costumam combinar com **Vale-Gás 2026: como consultar elegibilidade pelo CadÚnico**, ampliando o orçamento mensal."

#### LEIA TAMBÉM (caixa visual após P3)

A caixa "Leia também" é DIFERENTE — é bloco visual de sugestão de leitura adicional. Aceita backlinks **menos relacionados** que os embutidos no texto, sem forçar contexto. Pode ter 3-6 itens.
...
**REGRA EDITORIAL 2026-05-08 — VOZ DA REDAÇÃO, NUNCA CITAR OUTRO PORTAL/BLOG.**

O rodapé NÃO cita o veículo da fonte scrapeada (outro portal, blog ou site editorial). Citar concorrente entrega autoridade pra ele e sinaliza ao Google que somos agregador. Em vez disso, usar **voz primeira pessoa da redação** + verificação editorial em data atual.

✅ TEMPLATE OBRIGATÓRIO (sempre, mesmo sem URL da fonte):
```html
<p style='font-size:14px; color:#666;'>Levantamento da nossa redação, com verificação editorial em [mês] de [ano]. Para mais conteúdos atualizados, acompanhe as próximas publicações.</p>
```

Variações aceitas (alternar pra evitar fingerprint):
- `Apuração da nossa redação, com checagem editorial em [mês] de [ano].`
- `Monitoramento editorial da redação em [mês] de [ano] — atualizamos esta página conforme o tema evoluir.`
- `Reportagem da redação com base em fontes oficiais consultadas em [mês] de [ano].`

❌ NUNCA escrever:
- `Fonte: Informações publicadas pelo [Blog/Portal/Site X]`
- `Este conteúdo foi elaborado com base nas informações publicadas pelo [Blog/Portal/Site X]`
- `Transparência editorial: este conteúdo foi elaborado com base nas informações disponíveis nos portais [X, Y]`
- `verificadas pela redação com base no [Blog/Portal X]`

✅ EXCEÇÃO — fontes INSTITUCIONAIS OFICIAIS (gov.br, edu.br, INSS, MEC, INEP, Receita, Caixa, BCB, Dataprev, Senac, Senai, Sebrae, IBGE) podem aparecer no rodapé como citação de fonte autoritativa, NÃO como concorrente:
```html
<p style='font-size:14px; color:#666;'>Levantamento da nossa redação com base em dados oficiais do <a href='[URL_GOV]' target='_blank' rel='dofollow noopener'>[Órgão Oficial]</a>, verificados em [mês] de [ano].</p>
```
