...
/**
 * tick_filas_v2_cursosenac.php — substituto do tick_filas (pausado 2026-05-08).
 *
 ...
 * "redação CursoSenac Gratuito" (Maria Gusmão) + categoria via CategoryMatcher + featured Serper
 ...
 *   2. Filtra origens cursosenacgratuito (pingo:6,13,19,34,42)
 *   3. Score >= 5.0
...
// Origens cursosenacgratuito (G1 Educação, Vestibulando Web, MEC, Conecta Professores, Hora Brasil)
...
// Pega candidatas: status=aprovado, score>=5.5, origem cursosenac, ordenadas por score DESC, data DESC
$sqlCand = "SELECT id, score_discover, titulo, data_detectada FROM trends
            WHERE origem IN ({$qmOrig})
              AND status = 'aprovado'
              AND score_discover >= 5.0
              AND data_detectada >= NOW() - INTERVAL 7 DAY
...
echo "  candidatas aprovado >=5.0 (7d): " . count($candidatas) . "\n";
