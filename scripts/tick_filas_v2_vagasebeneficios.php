...
/**
 * tick_filas_v2_vagasebeneficios.php — substituto do tick_filas (pausado 2026-05-08).
 *
 ...
 *   2. Filtra origens vagasebeneficios (pingo:48,51,55-60)
 *   3. Score >= 5.5
...
// Origens vagasebeneficios (14 fontes)
...
// Pega candidatas: status=aprovado, score>=5.5, origem vagasebeneficios, ordenadas por score DESC, data DESC
...
// Filtra dedup
// Filtro de nicho: trend deve ter pelo menos 1 keyword do nicho vagas+benefícios
$keywordsNicho = '/\b(vaga|emprego|sal[aá]rio|inss|bolsa\s+fam[ií]lia|aux[ií]lio|benef[ií]cio|concurso|trabalho|contrata[cç][aã]o|carteira\s+assinada|aposentadoria|pis|fgts|seguro|cnh|cpf|13\s*[ªo]?\s*sal|pa[íi]s|brasileiro)/iu';

$processadas = 0;
foreach ($candidatas as $cand) {
    if ($processadas >= $alvoEstaRun) break;

    // Filtro nicho: descarta trend que não bate keywords
